...
"""Module for generating synthetic order data, handling validation, MinIO upload, and file registry updates."""
...
@dataclass
class CFG:
    """Configuration class for data generation and upload settings."""
...
def generate_df(cfg: CFG) -> pd.DataFrame:
    """Generate a DataFrame with synthetic order data."""
    fake = Faker()
    ...
    logger.info("Generated %d records (including duplicates)", len(df))
    return df
...
def write_csv(cfg: CFG, df: pd.DataFrame) -> Path:
    """Write DataFrame to a CSV file with timestamped name."""
    df_copy = df.copy()
    ...
    logger.info("Wrote CSV to %s", p)
    return p

class _MinioLazy:
    """Lazy initialization class for MinIO client."""
    ...
    def ensure_bucket(self, bucket: str):
        """Ensure the specified MinIO bucket exists."""
        c = self._client_get()
    ...
    def upload_file(self, local_path: Path, object_name: Optional[str] = None):
        ...
        logger.info("Uploaded file to MinIO as %s/%s", self.cfg.minio_bucket, obj)
        return obj

def upload_with_retries(cfg: CFG, local_path: Path) -> Tuple[bool, Optional[str]]:
    """Attempt to upload a file to MinIO with retry logic."""
    if not (cfg.minio_endpoint and cfg.minio_access and cfg.minio_secret):
        logger.warning("MinIO credentials/config incomplete; skipping upload")
        return False, None
    ...
    for attempt in range(1, cfg.minio_retries + 1):
        try:
            ...
            logger.warning("MinIO upload attempt %d/%d failed: %s", attempt, cfg.minio_retries, e)
            last_exc = e
            time.sleep(cfg.minio_backoff ** (attempt - 1))
    logger.error("MinIO upload failed after %d attempts: %s", cfg.minio_retries, last_exc)
    return False, None

def _get_db_engine():
    """Lazily retrieve the database engine instance."""
    try:
        from src.db.connection import get_db_engine
    except Exception as e:
        logger.warning("Could not import src.db.connection.get_db_engine: %s", e)
        return None
    try:
        engine = get_db_engine()
    except Exception as e:
        logger.warning("get_db_engine() failed: %s", e)
        return None
    return engine

def ensure_file_registry(engine) -> None:
    """Create the file_registry table if it does not exist."""
    ddl = """
...
def insert_file_record(engine, file_name: str, uploaded_at: datetime, status: str, rows_in: int, duplicate_count: int, validation_summary: Dict[str, Any]) -> Optional[int]:
    """Insert a new record into the file_registry table."""
    insert_sql = """
    ...
    with engine.connect() as conn:
        ...
        if row:
            record_id = int(row[0])
            logger.info("Inserted file_registry id=%s", record_id)
            return record_id
    return None
...
def update_file_record_processed(engine, record_id: int, processed_at: datetime, status: str, rows_out: Optional[int] = None, validation_summary: Optional[Dict[str, Any]] = None):
    """Update the processed status of a file_registry record."""
    update_sql = """
    ...
    logger.info("Updated file_registry id=%s -> %s", record_id, status)

def make_validation_summary_from_df(df: pd.DataFrame) -> Dict[str, Any]:
    """Generate a validation summary dictionary from a DataFrame."""
    total = len(df)
...
def call_project_upsert(local_file: Path) -> Any:
    """Execute the upsert operation on the provided local file."""
    try:
        from src.db.upsert import upsert_orders
    except Exception as e:
        logger.warning("Could not import src.db.upsert.upsert_orders: %s", e)
        return None
    try:
        logger.info("Calling upsert_orders(%s)", local_file)
        result = upsert_orders(str(local_file))
        return result
    except Exception as e:
        logger.exception("upsert_orders raised: %s", e)
        raise

def call_validator(csv_path: str) -> Tuple[bool, Optional[str]]:
    """Invoke the validator on the specified CSV file."""
    try:
        from src.processor.validate import validate_orders
    except Exception as e:
        logger.warning("Could not import src.processor.validate.validate_orders: %s", e)
        return False, f"validator not available: {e}"
    try:
        return validate_orders(csv_path)
    except Exception as e:
        logger.exception("Validator raised: %s", e)
        return False, str(e)
...
def run(
    *,
    ...
    """Execute the complete data generation and processing workflow."""
    ...
    try:
            ...
            for attempt in range(1, cfg.pg_retries + 1):
                try:
                    ...
                    if not is_valid:
                        ...
                        logger.error("Validation failed for %s: %s", out_path, validation_msg)
                        return 0
                    else:
                        record_id = insert_file_record(engine, out_path.name, datetime.utcnow(), "uploaded", vsum["rows_in"], vsum["duplicate_count"], vsum)
                    break
                except Exception as e:
                    logger.warning("Postgres attempt %d failed: %s", attempt, e)
                    time.sleep(cfg.pg_backoff ** (attempt - 1))
        else:
            logger.info("Database engine unavailable; skipping file_registry insertion")
            if not is_valid:
                logger.error("Validation failed but DB not available; stopping")
                return 0
        ...
        if upload:
            ...
            if not ok:
                logger.error("Upload failed for %s", out_path)
                if cfg.fail_on_upload_fail:
                    raise RuntimeError("MinIO upload failed and fail_on_upload_fail set")
            else:
                logger.info("Upload OK: %s", object_name)

        # Call upsert if requested
        if call_upsert_flag:
            try:
                ...
                logger.exception("Upsert/processing failed for %s: %s", out_path, e)
                if engine is not None and record_id is not None:
                    try:
                        ...
                        logger.error("Failed to update registry after processing error: %s", ue)

        logger.info("Processing completed. Local file at %s", out_path)
        return 0
    except Exception as e:
...
if __name__ == "__main__":
    """Handle command-line execution of the data generation workflow."""
    parser = argparse.ArgumentParser(description="Generate orders CSV, validate, optionally upload and call upsert.")
    parser.add_argument("--num", type=int, default=1000)
    parser.add_argument("--upload", action="store_true")
    parser.add_argument("--call-upsert", action="store_true")
